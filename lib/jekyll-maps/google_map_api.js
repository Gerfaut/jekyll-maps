...
var jekyllMaps = (function() {
  ...
  function render() {
    ...
    function markerPopup() {
      ...
      content += '<a href="' + this.url + '"><div class="map-img-background" style="background-image: url(' + this.image + ')">';
      content += '<h5 class="map-title bundle-title">' + this.title + '</h5>';
      content += '</div></a>';

      if (this.popup_html.length > 0) {

        var popup_content = this.popup_html;
        var maxLength = 200; // maximum number of characters to extract

        //Trim and re-trim only when necessary (prevent re-trim when string is shorted than maxLength, it causes last word cut) 
        if(popup_content.length > maxLength){
            //trim the string to the maximum length
            popup_content = popup_content.substr(0, maxLength);
        
            //re-trim if we are in the middle of a word and 
            popup_content = popup_content.substr(0, Math.min(popup_content.length, popup_content.lastIndexOf(" ")));

            popup_content += "...";
        }

        content += '<div class="map-desc">' + popup_content + '</div>';
      }

      content += '<a href="' + this.url + '" class="normal-btn">Lire la suite</a>';
